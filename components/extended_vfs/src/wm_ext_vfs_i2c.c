...
#define I2C_TRANSFER_TIMEOUT    (1000 / portTICK_RATE_MS)  /*!< I2C transfer timeout time(FreeRTOS ticks) */
...
static const char *TAG = "wm_ext_vfs_i2c";
...
static int i2c_master_exchange(int port, const i2c_ex_msg_t *ex_msg)
{
    ...
    if (ex_msg->flags & I2C_EX_MSG_DELAY_EN) {
        ticks = ex_msg->delay_ms / portTICK_RATE_MS;
        if (ex_msg->delay_ms % portTICK_RATE_MS) {
            ticks++;
        }
    } else {
        ticks = 0;
